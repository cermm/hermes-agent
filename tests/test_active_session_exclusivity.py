...
def test_a_recycled_pid_does_not_keep_a_lease_alive(legacy):
    ...
    if legacy:
        entries[0].pop('process_identity', None)
    else:
        # Newly written entries use the host fingerprint ahead of the legacy float.
        from hermes_cli.active_sessions import _process_identity
        observed = _process_identity(os.getpid())
        if observed is None:
            pytest.skip('host does not expose a stable process fingerprint')
        entries[0]['process_identity'] = observed + 1
    _write_entries(_state_path(), entries)
