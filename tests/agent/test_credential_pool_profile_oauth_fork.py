...
def test_borrowing_profile_load_pool_does_not_materialize_local_copy(fleet, legacy):
    from agent.credential_pool import load_pool

    fresh = _profile(fleet, "fresh")
    if legacy:
        (fresh / "auth.json").write_text(json.dumps({"providers": {}}))
    fleet["use"](fresh)
    pool = load_pool("anthropic")
    assert [e.id for e in pool.entries()] == ["abc123"]
    assert pool._borrowed_root_ids == ({"abc123"} if legacy else set())
    assert fleet["rows"](fresh) is None


@pytest.mark.parametrize("legacy", [False, True])
def test_borrower_prune_never_deletes_root_singleton_grant(fleet, tmp_path, legacy):
    ...
    if legacy:
        (kid / "auth.json").write_text(json.dumps({"providers": {}}))
    fleet["use"](kid)
    pool = load_pool("anthropic")
...
@pytest.mark.parametrize("authority", ["legacy", "profile"])
def test_profile_auth_add_owns_only_its_own_rows(fleet, authority):
    ...
    if authority == "legacy":
        ...
        (kid / "config.yaml").write_text("auth:\n  authority: profile\n")
    fleet["use"](kid)
    pool = load_pool("anthropic")
...
def test_heal_same_store_skip_is_memoized_off_the_hot_path(fleet, monkeypatch):
    ...
    root = fleet["root"]
    _seed_codex_grant(root)
    shared = _shared_profile(fleet, "shared", link=lambda target, alias: os.link(target, alias))
    fleet["use"](shared)
...
def _local_pkce_migration(fleet):
    from agent.credential_pool import load_pool
    from hermes_cli.auth_migration import plan_shared_migration

    kid = _profile(fleet, "migration")
    (kid / "config.yaml").write_text("auth:\n  authority: profile\n")
    (kid / "auth.json").write_text(json.dumps({"providers": {}, "active_provider": "anthropic"}))
    singleton = kid / ".anthropic_oauth.json"
    singleton.write_text(json.dumps({
        "accessToken": "sk-ant-oat01-AT0", "refreshToken": "sk-ant-ort-RT0", "expiresAt": 1,
    }))
    fleet["use"](kid)
    assert load_pool("anthropic").entries()[0].source == "hermes_pkce"
    # Only this profile owns the grant, so there is no conflicting root row.
    store = json.loads((fleet["root"] / "auth.json").read_text())
    store["credential_pool"].pop("anthropic")
    (fleet["root"] / "auth.json").write_text(json.dumps(store))
    fleet["use"](fleet["root"])
    return kid, plan_shared_migration(profile=kid.name)


@pytest.mark.parametrize("rollback", [False, True])
def test_shared_migration_preserves_profile_pkce_grant(fleet, rollback):
    from agent.credential_pool import load_pool
    from hermes_cli.auth_migration import apply_shared_migration, rollback_shared_migration, AuthMigrationError

    kid, plan = _local_pkce_migration(fleet)
    before = {name: (kid / name).read_bytes() for name in ("auth.json", ".anthropic_oauth.json", "config.yaml")}
    root_before = (fleet["root"] / "auth.json").read_bytes()
    apply_shared_migration(plan_id=plan.plan_id, plan_digest=plan.plan_digest, conflict_policy="abort")
    if rollback:
        assert rollback_shared_migration(plan_id=plan.plan_id) == "rolled_back"
        assert (fleet["root"] / "auth.json").read_bytes() == root_before
        assert all((kid / name).read_bytes() == raw for name, raw in before.items())
    fleet["use"](kid)
    selected = load_pool("anthropic").select()
    assert selected is not None and selected.access_token == "sk-ant-oat01-AT1"
    if not rollback:
        assert selected.source == "manual:hermes_pkce"
        assert (kid / "auth.json").read_bytes() == before["auth.json"]
        assert (kid / ".anthropic_oauth.json").read_bytes() == before[".anthropic_oauth.json"]
        assert not (fleet["root"] / ".anthropic_oauth.json").exists()
        fleet["use"](fleet["root"])
        assert load_pool("anthropic").select().access_token == "sk-ant-oat01-AT1"
        with pytest.raises(AuthMigrationError, match="changed after migration"):
            rollback_shared_migration(plan_id=plan.plan_id)
    assert [event[0] for event in fleet["server"]["log"]] == ["ROTATE"]


def test_shared_migration_rejects_changed_singleton_snapshot(fleet):
    from hermes_cli.auth_migration import apply_shared_migration, AuthMigrationError

    kid, plan = _local_pkce_migration(fleet)
    root_before = (fleet["root"] / "auth.json").read_bytes()
    (kid / ".anthropic_oauth.json").write_text(json.dumps({"accessToken": "changed-after-plan"}))
    with pytest.raises(AuthMigrationError, match="inputs changed"):
        apply_shared_migration(plan_id=plan.plan_id, plan_digest=plan.plan_digest, conflict_policy="abort")
    assert (fleet["root"] / "auth.json").read_bytes() == root_before
    assert "authority: profile" in (kid / "config.yaml").read_text()


@pytest.mark.parametrize("problem", ["missing", "incomplete", "ambiguous", "spent"])
def test_shared_migration_rejects_unusable_singleton_before_mutation(fleet, problem):
    from hermes_cli.auth_migration import plan_shared_migration, AuthMigrationError

    kid, _plan = _local_pkce_migration(fleet)
    singleton = kid / ".anthropic_oauth.json"
    if problem == "missing":
        singleton.unlink()
    elif problem == "incomplete":
        singleton.write_text(json.dumps({"accessToken": "fake"}))
    elif problem == "ambiguous":
        store = json.loads((kid / "auth.json").read_text())
        store["credential_pool"]["anthropic"] *= 2
        (kid / "auth.json").write_text(json.dumps(store))
    else:
        from agent.anthropic_credentials import mark_rotation_consumed_uncommitted
        mark_rotation_consumed_uncommitted("sk-ant-ort-RT0", source_path=singleton)
    before = (fleet["root"] / "auth.json").read_bytes()
    with pytest.raises(AuthMigrationError, match="first"):
        plan_shared_migration(profile=kid.name)
