...
def _make_runner_with_adapter(session_id: str=None):
    ...
    src = _make_source()
    session_entry = SessionEntry(session_key=build_session_key(src), session_id=session_id or f'goal-sess-{uuid.uuid4().hex[:8]}', created_at=datetime.now(), updated_at=datetime.now(), platform=Platform.TELEGRAM, chat_type='dm')
...
@pytest.mark.asyncio
async def test_replaced_goal_generation_remains_in_shutdown_drain_until_done(hermes_home):
...
@pytest.mark.asyncio
async def test_goal_control_remains_owned_through_post_evaluation_notice(hermes_home, monkeypatch):
    ...
    monkeypatch.setattr(runner, '_defer_goal_status_notice_after_delivery', _blocked_notice)
    task = None
    try:
        with patch('hermes_cli.goals.judge_goal', return_value=('continue', 'needs another turn', False, None, False)):
            task = asyncio.create_task(runner._post_turn_goal_continuation(session_entry=session_entry, source=src, final_response='partial result'))
            await asyncio.wait_for(notice_started.wait(), timeout=3.0)
            assert runner._active_goal_run_count() == 1
...
@pytest.mark.asyncio
async def test_shutdown_removes_generation_owned_post_delivery_goal_notice(hermes_home):
    runner, adapter, session_entry, src = _make_runner_with_adapter()
    ...
    with patch('hermes_cli.goals.judge_goal', return_value=('done', 'complete', False, None, False)):
        await runner._post_turn_goal_continuation(session_entry=session_entry, source=src, final_response='finished')
    session_key = session_entry.session_key
    assert runner._active_goal_run_count() == 1
...
@pytest.mark.asyncio
async def test_post_delivery_goal_notice_retires_control_only_after_callback(hermes_home):
    runner, adapter, session_entry, src = _make_runner_with_adapter()
    from gateway.platforms.base import BasePlatformAdapter
    from hermes_cli.goals import GoalManager
    adapter._post_delivery_callbacks = {}
    ...
    with patch('hermes_cli.goals.judge_goal', return_value=('done', 'complete', False, None, False)):
        await runner._post_turn_goal_continuation(session_entry=session_entry, source=src, final_response='finished')
    callback = adapter.pop_post_delivery_callback(session_entry.session_key, generation=0)
...
@pytest.mark.asyncio
async def test_cancelled_goal_notice_preserves_foreign_post_delivery_callback(hermes_home):
    runner, adapter, session_entry, src = _make_runner_with_adapter()
    from gateway.platforms.base import BasePlatformAdapter
    from hermes_cli.goals import GoalManager
    adapter._post_delivery_callbacks = {}
    ...
    adapter.register_post_delivery_callback(session_key, _foreign_callback, generation=0)
...
@pytest.mark.asyncio
async def test_post_evaluation_cancellation_does_not_interrupt_reused_worker(hermes_home, monkeypatch):
    ...
    def _judge_goal(*_args, **_kwargs):
        ...
        return ('continue', 'needs another turn', False, None, False)
    monkeypatch.setattr(runner, '_defer_goal_status_notice_after_delivery', _blocked_notice)
    task = None
    try:
        with patch('hermes_cli.goals.judge_goal', side_effect=_judge_goal):
            ...
            runner._cancel_all_goal_runs()
            reused_thread_id, observed_interrupt = await runner._run_in_executor_with_context(_observe_reused_worker)
