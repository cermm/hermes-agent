...
Run `npm test` after changing the dependency graph. It exercises the installed
Photon telemetry wrapper with a local HTTP receiver, checks trace/log/metric
export, and checks provider shutdown without external credentials. The 0.x
OpenTelemetry log and exporter packages are kept on the compatible 0.218 family:
0.221 changes constructor/exporter contracts that the current Photon wrapper
does not support. A successful import or dependency-tree check alone does not
verify these runtime contracts.
