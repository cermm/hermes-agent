...
class BasePlatformAdapter(ABC):
    ...
    def register_post_delivery_callback(
        ...
        *,
        ...
        ``on_discard`` releases retained bookkeeping if a newer registration
        replaces this callback before delivery. Popped callbacks own their cleanup.
        """
        ...
        if normalized_generation is not None and generation_entries:
            ...
            if normalized_generation < max_generation:
                return None
            if normalized_generation > max_generation:
                discarded = [entry for entry in entries
                             if entry.get("generation") is not None
                             and int(entry["generation"]) != normalized_generation]
                entries = [
                    entry
                ...
                # These callbacks were never popped for delivery. Release only
                # their retained bookkeeping; a running callback owns its finally.
                for entry in discarded:
                    retire = entry.get("on_discard")
                    if callable(retire):
                        try:
                            retire()
                        except Exception:
                            logger.exception("post-delivery callback retirement failed")

        if normalized_generation is not None:
            # Legacy callbacks registered before the handler binds a generation
            # belong to that delivery, as in the original chained-callback API.
            entries = [
                {**entry, "generation": normalized_generation}
                if entry.get("generation") is None else entry
                for entry in entries
            ]
        entries.append(
            {
