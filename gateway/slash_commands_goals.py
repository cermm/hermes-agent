...
class GatewayGoalCommandsMixin:
    ...
    async def _handle_goal_command(self, event: MessageEvent) -> str:
        ...
        if lower == "unwait":
            self._clear_goal_continuations(event, "unwait")
    ...
    def _clear_goal_continuations(self, event: MessageEvent, verb: str) -> None:
        try:
            ...
            if quick_key:
                self._cancel_goal_run(quick_key)
    ...
    def _goal_resume(self, mgr, event: MessageEvent) -> str:
        self._clear_goal_continuations(event, "resume")
        state = mgr.resume()
        if state is None:
    ...
    def _goal_wait(self, mgr, wait_arg: str, event: MessageEvent) -> str:
        ...
        self._clear_goal_continuations(event, "wait")
    ...
    def _goal_gate(self, mgr, gate_arg: str, event: MessageEvent) -> str:
        ...
        if gate_lower.startswith("add "):
            ...
            self._clear_goal_continuations(event, "gate add")
        ...
        if gate_lower.startswith(("remove ", "rm ")):
            self._clear_goal_continuations(event, "gate remove")
        ...
        if gate_lower == "clear":
            self._clear_goal_continuations(event, "gate clear")
    ...
    async def _goal_set(self, mgr, args: str, lower: str, event: MessageEvent) -> str:
        ...
        try:
            self._clear_goal_continuations(event, "set")
