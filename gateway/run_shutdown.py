...
class GatewayShutdownMixin:
    ...
    def _drain_work_counts(self) -> tuple:
        """``(agents, cron, api, deferred)``; deferred includes goal gates and notices."""
        return (
            self._running_agent_count(), self._active_cron_job_count(),
            self._active_api_run_count(), self._active_deferred_agent_worker_count() + self._active_goal_run_count(),
        )
