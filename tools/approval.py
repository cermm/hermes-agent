...
_gateway_notify_tokens: dict[str, object] = {}
...
def resolve_gateway_approval(session_key: str, choice: str,
    ...
    *resolve_all* resolves every pending approval (``/approve all``); otherwise the oldest
    ...
    with _lock:
        ...
        if not queue:
            _gateway_queues.pop(session_key, None)
        # Queue removal and its explicit decision are one state transition: a
        # notifier racing a fast reply must distinguish consent from teardown.
...
def _deferred_command_session_key(command: str) -> str:
    """Session-only approval key for an exact deferred terminal command."""
    digest = hashlib.sha256(command.encode("utf-8", errors="surrogatepass")).hexdigest()
    return f"deferred-terminal-command:{digest}"

def _is_session_approved(session_key: str, pattern_key: str) -> bool:
    """Check only session-scoped approval, ignoring permanent allowlists."""
...
def _gateway_notify_registration(session_key: str):
    """Atomically capture both the notification route and the registration owning it."""
    with _lock:
        return _gateway_notify_cbs.get(session_key), _gateway_notify_tokens.get(session_key)
...
def _pending_result(spec, session_key: str, *, command: str, description: str,
                    ...
                    smart_denied: bool, required_session_key: str | None = None) -> dict:
    ...
    if required_session_key is not None:
        pending.update(pattern_key=required_session_key, pattern_keys=[required_session_key],
    ...
    if required_session_key is not None:
        result.update(allow_permanent=False, allow_session=True)
    return result
...
def _human_decision(spec: _GateSpec, *, command: str, description: str,
                    ...
                    permanent_capable: bool = True, pending_body=None, required_session_key: str | None = None) -> dict:
    ...
    allow_permanent = permanent_capable and not smart_denied and required_session_key is None
    ...
    def grant(choice: str) -> dict:
        if required_session_key is not None:
            if choice != "session":
                return _denied(
                    "BLOCKED: Deferred command execution requires an explicit session-scoped approval. "
                    "A one-operation or permanent choice does not authorize an automatic goal gate.",
                    pattern_key=pattern_key, description=description, outcome="session_authorization_required",
                )
            approve_session(session_key, required_session_key)
    ...
    if is_gateway or is_ask:
        ...
        notify_cb, notify_token = _gateway_notify_registration(session_key)
        if notify_cb is not None:
            ...
            data = {
                ...
                # Redaction can make distinct secret-bearing commands look the
                # same. Coalesce on opaque raw identity as well as display text.
                "command_identity": hashlib.sha256(command.encode("utf-8", errors="surrogatepass")).hexdigest(),
                "pattern_keys": pattern_keys, "description": display_description,
                "allow_permanent": allow_permanent,
                "allow_session": not smart_denied,
            }
            if required_session_key is not None:
                data["requires_session"] = True
            if smart_denied:
                data["smart_denied"] = True
            decision = _await_gateway_decision(session_key, notify_cb, data, surface="gateway", owner_token=notify_token)
...
def check_all_command_guards(command: str, env_type: str,
                             ...
                             require_explicit_authorization: bool | None = None) -> dict:
    ...
    if require_explicit_authorization is None:
        require_explicit_authorization = approval_context.deferred_command_session_authorization_required()
    if not require_explicit_authorization and _should_skip_container_guards(env_type, has_host_access=has_host_access):
        return _approved()
    ...
    if not require_explicit_authorization and (_yolo_active() or approval_mode == "off"):
        return _approved()
    if not require_explicit_authorization and _command_matches_permanent_allowlist(command):
        return _approved()
    ...
    if require_explicit_authorization and not (is_cli or is_gateway or is_ask):
        return _blocked("BLOCKED: Deferred command execution requires explicit session-scoped approval, "
                        "but no user approval surface is present.")
    ...
    warnings = []
    session_key = get_current_session_key()
    deferred_key = _deferred_command_session_key(command) if require_explicit_authorization else None
    if deferred_key is not None and _is_session_approved(session_key, deferred_key):
        return _approved()

    def authorized(key: str) -> bool:
        return not require_explicit_authorization and is_approved(session_key, key)

    if tirith_result["action"] in {"block", "warn"}:
        findings = tirith_result.get("findings") or []
        seen = set()
        for finding in findings or [{}]:
            raw_id = finding.get("rule_id") if isinstance(finding, dict) else None
            rule_id = str(raw_id).strip() if raw_id is not None else ""
            key = f"tirith:{rule_id or 'unknown'}"
            if key in seen or authorized(key):
                continue
            seen.add(key)
            detail = dict(tirith_result, findings=[finding] if findings else [])
            warnings.append((key, _format_tirith_description(detail), True))
    if is_dangerous and not authorized(pattern_key):
        warnings.append((pattern_key, description, False))
    if deferred_key is not None and not warnings:
        warnings.append((deferred_key, "deferred goal terminal command execution requires "
                         "session-scoped approval for this exact command", False))
    if not warnings:
        return _approved()
    ...
    return _human_decision(
        ...
        is_cli=is_cli, is_gateway=is_gateway, is_ask=is_ask, smart=approval_mode == "smart" and not require_explicit_authorization,
        permanent_capable=any(not is_t for _, _, is_t in warnings), required_session_key=deferred_key,
    )
