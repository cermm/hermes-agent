...
class _ApprovalEntry:
    ...
    __slots__ = ("event", "data", "result", "reason", "acknowledged", "owner_token")
...
def _await_gateway_decision(session_key: str, notify_cb, approval_data: dict, *, surface: str = "gateway", owner_token=None) -> dict:
    ...
    def stale_owner() -> bool:
        return owner_token is not None and (
            _approval._gateway_notify_tokens.get(session_key) is not owner_token
            or _approval._gateway_notify_cbs.get(session_key) is not notify_cb
        )

    stale_result = {"resolved": False, "choice": None, "notify_failed": True, "stale_owner": True}
    keys = list(approval_data.get("pattern_keys") or [])
    with _approval._lock:
        if stale_owner():
            return stale_result
        leader = next((e for e in _approval._gateway_queues.get(session_key, [])
                       if e.owner_token is owner_token
                        and e.data.get("command") == approval_data.get("command")
                        and e.data.get("command_identity") == approval_data.get("command_identity")
                        and bool(e.data.get("requires_session")) == bool(approval_data.get("requires_session"))
                        and e.data.get("allow_session", True) == approval_data.get("allow_session", True)
                        and e.data.get("allow_permanent", True) == approval_data.get("allow_permanent", True)
    ...
    with _approval._lock:
        stale = stale_owner()
        already_resolved = not stale and entry.result is not None
        stale = stale or (not already_resolved and owner_token is not None
                          and entry not in _approval._gateway_queues.get(session_key, []))
    if already_resolved:
        return _finish(payload, True, entry.result, entry.reason)
    if stale:
        _drop_entry()
        _ctx._fire_approval_hook("post_approval_response", **payload, choice="stale_owner")
        return stale_result
