...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    while i < len(argv):
        arg = argv[i]
        if arg == "auth" and argv[i + 1:i + 2] == ["migrate-shared"]:
            # This command owns --profile as the migration source selector.
            # A global profile override can still precede `auth`.
            break
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    try:
        ...
        auth_path = None
    if auth_path is not None and _auth_store_logged_in(auth_path, PROVIDER_REGISTRY, strict_profile_scope):
        return True
...
def cmd_chat(args):
    ...
    from hermes_cli.result_metadata_argv import _claim_result_metadata_args, _publish_unknown_result_metadata
...
def _cmd_chat(args):
    ...
    from hermes_cli.result_metadata_argv import _validate_result_metadata_args

    metadata_requested = getattr(args, "result_meta_fd", None) is not None
    if metadata_requested and not getattr(args, "_result_meta_query_validated", False):
        _read_query_file(args)
    use_tui = _validate_result_metadata_args(args) if metadata_requested else None
...
def _main_impl(early_owner=None, metadata_state=None):
    ...
    if early_owner is None and _try_termux_fast_tui_launch():
        return
    if early_owner is None and _try_termux_fast_cli_launch():
        return
    if early_owner is None and _try_fast_serve_launch():
        return
    if early_owner is None and _try_fast_chat_launch():
        return
    ...
    if container_info and early_owner is not None:
        early_owner.close()
        print("Error: --result-meta-fd cannot be forwarded into a container.", file=sys.stderr)
        raise SystemExit(2)
    if container_info:
    ...
    if early_owner is not None:
        ...
        args.result_meta_fd = early_owner
        _read_query_file(args)
        _validate_result_metadata_args(args)
        if metadata_state is not None:
            metadata_state["query_validated"] = True
...
def main():
    """Claim automation output before any startup side effects."""
    from hermes_cli.result_metadata_argv import (
        _early_claim_result_metadata_argv,
        _publish_result_metadata_preparser_failure,
        _publish_unknown_result_metadata,
    )

    argv = _coalesce_session_name_args(sys.argv[1:])
    claim = _early_claim_result_metadata_argv(argv, include_parser_state=True)
    if claim is None:
        return _main_impl()
    owner, parser, chat_parser = claim
    metadata_state = {"query_validated": False}
    try:
        result = _main_impl(early_owner=owner, metadata_state=metadata_state)
    except BaseException as exc:
        _publish_result_metadata_preparser_failure(
            owner, argv, exc, parser, chat_parser,
            query_validated=metadata_state["query_validated"],
        )
    else:
        _publish_unknown_result_metadata(owner)
        return result
