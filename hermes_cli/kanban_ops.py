...
def _cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        _print_json({
            ...
            "effect_status": "unknown" if res.effect_unknown else ("spawned" if res.spawned else "not_spawned"),
            "effect_unknown": res.effect_unknown,
            "no_spawn_reason": "dispatch_locked" if res.skipped_locked else res.no_spawn_reason,
            "board": kb.get_current_board(),
            "selection_rejected": res.selection_rejected,
            **{k: getattr(res, k)
               for k in ("reclaimed", "crashed", "timed_out", "stale", "auto_blocked", "promoted")},
            "spawned": [
                {"task_id": tid, "assignee": who, "workspace": ws, "run_id": res.spawned_run_ids.get(tid)} for (tid, who, ws) in res.spawned
            ],
