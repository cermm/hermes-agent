...
def _resolve_project_link(
    ...
    workspace_kind: str, workspace_path: Optional[str], *, board: Optional[str] = None,
    ...
    if project_obj is None and project_source_task_id:
        project_obj, project_repo = _project_from_source_task(
            conn, _pdb, project_id, str(project_source_task_id), board=board,
        )
...
def _project_from_source_task(
    conn: sqlite3.Connection, _pdb: Any, project_id: str, source_task_id: str, *, board: Optional[str] = None,
    ...
    reusing the source task's literal worktree path. External worktrees require
    materialized Git source/branch evidence; legacy repo-local anchors retain
    their existing recovery contract."""
    ...
    if not (
        ...
        return None, None
    from hermes_cli.kanban_db_workspace import _project_source_worktree_repo

    source_repo = _project_source_worktree_repo(source_task, board=board)
...
def create_task(
    ...
    project_id, project_obj, project_repo, workspace_kind = _resolve_project_link(
        conn, project_id, project_source_task_id, workspace_kind, workspace_path, board=board
    )
    ...
    if idempotency_key:
        ...
        if row:
            from hermes_cli.kanban_db_workspace import _reconcile_configured_worktree_target

            existing = get_task(conn, row["id"])
            if existing is not None:
                _reconcile_configured_worktree_target(conn, existing, board=board)
            return row["id"]
    ...
    for attempt in range(2):
        ...
        if workspace_kind == "worktree":
            from hermes_cli.kanban_db_workspace import _configured_create_worktree_target

            configured_target = _configured_create_worktree_target(workspace_path=workspace_path,
                project_repo=project_repo, project_id=project_id, board=board, task_id=task_id)
            ...
            with write_txn(conn, allow_nested=True):
                ...
                if project_obj is not None and workspace_kind == "worktree":
                    if project_repo and not insert_workspace_path:
                        insert_workspace_path = os.path.join(project_repo, ".worktrees", task_id)
                    if not branch_name:
                ...
                conn.execute(
                    ...
                    (
                        ...
                        created_by, now, workspace_kind, insert_workspace_path,
                        branch_name, project_id, tenant, idempotency_key,
