...
def run_gate(gate: GoalGate, *, cwd: Optional[str] = None) -> Tuple[bool, int, str]:
    ...
    from tools.approval_context import deferred_command_session_authorization_required
    if deferred_command_session_authorization_required():
        from hermes_cli.goal_gate_execution import run_deferred_goal_gate
        return run_deferred_goal_gate(gate.command, timeout=gate.timeout_seconds,
                                      cwd=cwd, tail_chars=_GATE_OUTPUT_TAIL_CHARS)
    try:
...
class GoalManager:
    ...
    def __init__(self, session_id: str, *, default_max_turns: int = DEFAULT_MAX_TURNS):
        ...
        self._evaluation_ownership = None
    ...
    def _save(self) -> Optional[GoalState]:
        if self._evaluation_ownership is not None:
            self._evaluation_ownership.save(self._state)
        else:
    ...
    def evaluate_after_turn(
        ...
        background_processes: Optional[List[Dict[str, Any]]] = None,
        is_current=None, write_lock=None,
    ) -> Dict[str, Any]:
        """Evaluate the loaded goal, refusing writes after cancellation or replacement."""
        from hermes_cli.goal_evaluation import GoalEvaluationOwnership, GoalEvaluationSuperseded

        if self._state is None or self._state.status != "active":
            return _decision(self._state.status if self._state else None, False, None,
                             "inactive", "no active goal", "")
        try:
            self._evaluation_ownership = GoalEvaluationOwnership(
                _get_session_db(), _meta_key(self.session_id), self._state,
                is_current=is_current, write_lock=write_lock,
            )
            return self._evaluate_after_turn(
                last_response, user_initiated=user_initiated,
                background_processes=background_processes,
            )
        except GoalEvaluationSuperseded:
            self._state = load_goal(self.session_id)
            return _decision(self._state.status if self._state else None, False, None,
                             "superseded", "goal evaluation no longer owns state", "")
        finally:
            self._evaluation_ownership = None
