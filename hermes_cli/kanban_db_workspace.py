...
def _configured_worktree_root() -> Optional[Path]:
    """Opt-in profile policy; environment variables cannot bypass placement."""
    from hermes_cli.config import load_config_readonly

    settings = load_config_readonly().get("kanban") or {}
    if not isinstance(settings, dict):
        raise ValueError("kanban config must be a mapping")
    raw = settings.get("worktree_root")
    if raw is None or not str(raw).strip():
        return None
    root = Path(str(raw).strip()).expanduser()
    if not root.is_absolute():
        raise ValueError("kanban.worktree_root must be an absolute path")
    root = root.resolve(strict=False)
    for forbidden in (Path("/tmp"), Path("/mnt/c")):
        if root == forbidden or forbidden in root.parents:
            raise ValueError(f"kanban.worktree_root must not be under {forbidden}: {root}")
    return root


def _remote_owner_repo_namespace(repo_root: Path) -> str:
    remote = _kb._git_out(repo_root, "remote", "get-url", "origin")
    if not remote:
        raise ValueError("kanban.worktree_root requires remote.origin.url for its owner/repository namespace")
    if "://" in remote:
    ...
    if len(parts) < 2:
        raise ValueError("unable to derive owner/repository from remote.origin.url")
...
def _configured_worktree_namespace(repo_root: Path, policy_root: Path) -> Path:
    ...
    namespace = (root / _remote_owner_repo_namespace(repo_root)).resolve(strict=False)
    if namespace.parent != root:
        raise ValueError("configured worktree namespace escapes kanban.worktree_root")
    if _repo_root_for_worktree_target(namespace) is not None:
        raise ValueError(f"kanban.worktree_root must be outside every git repository: {root}")
    return namespace
...
def _repo_root_from_linked_worktree(path: Path) -> Optional[Path]:
    ...
    if common is not None and common.name == ".git":
        return _git_toplevel(common.parent)
    return None


def _configured_source_repo(
    *, requested: Optional[Path], board: Optional[str],
    project_id: Optional[str] = None, project_repo: Optional[str] = None,
) -> Path:
    # A target cannot vouch for its own identity when the board/project names
    # the source: two clones can have an identical remote and namespace.
    if project_repo:
        source = _git_toplevel(Path(project_repo).expanduser())
        if source is None:
            raise ValueError("explicit project primary repository is missing or is not a Git repository")
        return source
    if project_id:
        ...
        if project is not None:
            if not project.primary_path:
                raise ValueError("explicit project has no primary repository")
            source = _git_toplevel(Path(project.primary_path).expanduser())
            if source is None:
                raise ValueError("explicit project primary repository is missing or is not a Git repository")
            return source
    board_slug = board if board else _kb.get_current_board()
    raw = (_kb.read_board_metadata(board_slug).get("default_workdir") or "").strip()
    if raw:
        anchor = Path(raw).expanduser()
        if not anchor.is_absolute():
            raise ValueError(f"board {board_slug!r} default_workdir must be absolute")
        source = _git_toplevel(anchor)
        if source is None:
            raise ValueError(f"board {board_slug!r} default_workdir is not a git repository")
        return source
    if requested is not None and requested.is_absolute():
        source = _repo_root_from_linked_worktree(requested)
        if source is not None:
            return source
        source = _git_toplevel(requested)
        if source is not None and requested.resolve(strict=False) == source:
            return source
    raise ValueError("kanban.worktree_root requires a board/project source repository or repo-root anchor")


def _project_source_worktree_repo(task: Task, *, board: Optional[str]) -> Optional[Path]:
    """Recover project provenance from a recorded source task, never a remote slug alone."""
    path = Path(task.workspace_path)
    if not path.is_absolute() or path.name != task.id:
        return None
    if path.parent.name == ".worktrees":
        # Preserve the pre-existing repo-local, not-yet-materialized contract.
        return path.parent.parent
    source = _repo_root_from_linked_worktree(path)
    if source is None:
        raise ValueError("external project source worktree must be materialized before cross-profile recovery")
    if not task.branch_name or _git_current_branch(path) != task.branch_name:
        raise ValueError("external project source worktree branch contradicts its recorded task")
    authority = _configured_source_repo(requested=path, board=board)
    common = _git_common_dir(source)
    if common is None or _git_common_dir(path) != common or _git_common_dir(authority) != common:
        raise ValueError("external project source worktree contradicts the known source repository")
    policy_root = _configured_worktree_root()
    if policy_root is not None:
        _configured_worktree_target(workspace_path=str(path), project_repo=str(source),
            project_id=None, board=board, task_id=task.id, policy_root=policy_root)
    return source


def _configured_worktree_target(
    *, workspace_path: Optional[str], project_repo: Optional[str],
    project_id: Optional[str], board: Optional[str], task_id: str, policy_root: Path,
) -> tuple[Path, Path]:
    requested = Path(workspace_path).expanduser() if workspace_path else None
    source = _configured_source_repo(requested=requested, board=board,
                                     project_id=project_id, project_repo=project_repo)
    namespace = _configured_worktree_namespace(source, policy_root)
    if requested is None or requested.resolve(strict=False) == source:
        requested = namespace / task_id
    if not requested.is_absolute():
        raise ValueError("configured Kanban worktree path must be absolute")
    target = requested.resolve(strict=False)
    if target.parent != namespace or not target.name:
        raise ValueError(f"Kanban worktree path must resolve as a direct child of configured namespace {namespace}")
    return target, source


def _configured_create_worktree_target(
    *, workspace_path: Optional[str], project_repo: Optional[str],
    project_id: Optional[str], board: Optional[str], task_id: str,
) -> Optional[Path]:
    policy_root = _configured_worktree_root()
    if policy_root is None:
        return None
    return _configured_worktree_target(workspace_path=workspace_path, project_repo=project_repo,
        project_id=project_id, board=board, task_id=task_id, policy_root=policy_root)[0]


def _reconcile_configured_worktree_target(conn: sqlite3.Connection, task: Task, *, board: Optional[str]) -> None:
    if task.workspace_kind != "worktree":
        return
    target = _configured_create_worktree_target(workspace_path=task.workspace_path,
        project_repo=None, project_id=task.project_id, board=board, task_id=task.id)
    if target is None or str(target) == task.workspace_path:
        return
    # Git/config planning ran without the DB lock. A worker may have claimed or
    # an operator may have edited the task meanwhile; neither can be relabelled.
    with _kb.write_txn(conn, allow_nested=True):
        changed = conn.execute(
            "UPDATE tasks SET workspace_path = ? WHERE id = ? AND workspace_kind = 'worktree' "
            "AND workspace_path IS ? AND project_id IS ? AND branch_name IS ? "
            "AND status = ? AND claim_lock IS NULL",
            (str(target), task.id, task.workspace_path, task.project_id, task.branch_name, task.status),
        ).rowcount
        if changed != 1 or task.claim_lock is not None:
            raise RuntimeError("task changed or was claimed while applying configured worktree policy")


def _resolve_configured_worktree(task: Task, *, board: Optional[str], policy_root: Path, branch_name: str) -> tuple[Path, str]:
    target, source = _configured_worktree_target(workspace_path=task.workspace_path,
        project_repo=None, project_id=task.project_id, board=board, task_id=task.id, policy_root=policy_root)
    if target.exists():
        if not _is_linked_worktree_checkout(target):
            raise ValueError(f"configured Kanban worktree target {target} is not a linked git worktree")
        source_common = _git_common_dir(source)
        if source_common is None or source_common != _git_common_dir(target):
            raise ValueError(f"configured Kanban worktree target {target} belongs to a different git repository")
        if _git_current_branch(target) == branch_name:
            return target, branch_name
        # Preserve current sibling isolation for inherited paths, using the
        # external policy namespace instead of falling back inside the repo.
        fallback = target.parent / task.id
        if fallback == target:
            raise ValueError(f"configured Kanban worktree target {target} is occupied by another branch")
        copied = replace(task, workspace_path=str(fallback))
        return _resolve_configured_worktree(copied, board=board, policy_root=policy_root, branch_name=branch_name)
    _ensure_git_worktree(source, target, branch_name)
    # Another board can publish the same explicit destination between the
    # existence check and git-add's existing-target fast path.
    source_common = _git_common_dir(source)
    if (not _is_linked_worktree_checkout(target) or source_common is None
            or _git_common_dir(target) != source_common
            or _git_current_branch(target) != branch_name):
        raise ValueError(f"configured Kanban worktree target {target} changed during materialization")
    return target, branch_name
...
def _resolve_worktree_workspace(task: Task, *, board: Optional[str] = None) -> tuple[Path, str]:
    ...
    policy_root = _configured_worktree_root()
    if policy_root is not None:
        return _resolve_configured_worktree(task, board=board, policy_root=policy_root, branch_name=branch_name)
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    (raises if unset rather than guessing). With ``kanban.worktree_root`` set,
    placement is restricted to ``<root>/<remote-owner>-<remote-repo>/<leaf>``
    and existing targets must belong to the board/project source repository.
    Persist via ``set_workspace_path``.
    """
