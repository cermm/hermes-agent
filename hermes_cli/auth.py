...
def _auth_file_path() -> Path:
    ...
    pinned_path = transaction.path if transaction is not None else None
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    if transaction is not None:
        return transaction.fallback_path
    try:
...
def _auth_store_lock(
    ...
    """Lock the selected authority and fallback before shared Nous token locks."""
    from hermes_cli.auth_store_locks import _auth_store_locks

    if target_path is None:
        with _auth_store_locks(include_legacy_fallback=True, timeout_seconds=timeout_seconds):
            yield
    else:
        with _auth_store_locks([target_path], transaction_target=_auth_file_path(),
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None,
                     updated_at: Optional[str] = None) -> Path:
...
def _save_provider_state_to_source(
    ...
    if source_path is None or _same_path(source_path, _auth_file_path()):
        ...
        transaction = current_auth_transaction()
        if transaction is None or source_path.resolve(strict=False) not in transaction.locked_paths:
            raise RuntimeError(f"Refusing unlocked auth source write: {source_path}")
