...
def _read_entries(path: Path, *, strict: bool = False) -> list[dict[str, Any]]:
    ...
    for entry in valid:
        ...
        for bad, what in (
            ...
            (lambda: not _valid_process_identity(entry.get("process_identity")), "an invalid process identity"),
        ):
            if bad():
...
def _process_identity(pid: int) -> Optional[int]:
    """Return the host's stable start-time fingerprint without signalling the process."""
    try:
...
def _process_start_time_tolerance() -> float:
    """One platform clock tick accommodates legacy psutil float observations."""
    try:
        ticks = int(os.sysconf("SC_CLK_TCK"))
    except (AttributeError, OSError, TypeError, ValueError):
        return 0.001
    return max(0.001, 1.0 / ticks) if ticks > 0 else 0.001
...
def _pid_liveness(pid: Any, process_start_time: Any = None, process_identity: Any = None, *, lenient: bool = False) -> Optional[bool]:
    ...
    if process_identity is not None and process_identity != "":
        ...
        return current_identity == int(process_identity)
...
def _pid_alive(pid: Any, process_start_time: Any = None, process_identity: Any = None) -> bool:
    """Compatibility predicate; strict lease coordination uses tri-state liveness."""
    return bool(_pid_liveness(pid, process_start_time, process_identity, lenient=True))
...
def _prune_dead(entries: list[dict[str, Any]], *, strict: bool = False) -> list[dict[str, Any]]:
    ...
    for entry in entries:
        ...
        state = _pid_liveness(
            entry.get("pid"), entry.get("process_start_time"), entry.get("process_identity"), lenient=not tracked
        )
        if state is None:
