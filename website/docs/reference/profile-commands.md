...
| `--auth-mode <shared\|profile>` | Select the new profile's credential authority. `shared` is the safe default and does not copy OAuth `auth.json`; `profile` creates an isolated local authority and, when cloning, explicitly copies the resolved source authority under canonical locks. |
