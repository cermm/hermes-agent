...
- `hermes profile create NAME --auth-mode profile` creates an empty profile-local authority. Clone options never copy `auth.json` OAuth or credential-pool state into it; use an independent login or an explicit encrypted profile restore to populate it. Other clone-selected files such as `.env` retain their documented behavior.
