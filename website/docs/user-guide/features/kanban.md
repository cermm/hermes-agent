...
| `kanban.worktree_root` | unset | Optional profile root for Git worktrees. Targets must be direct children of `<root>/<remote-owner>-<remote-repo>/`, derived from `remote.origin.url`. Existing targets must belong to the board/project source repository. Unset preserves legacy placement. |
...
Set the value with `hermes config set kanban.worktree_root /absolute/path`. The root must be outside every Git repository and outside `/tmp` and `/mnt/c`; symlink escapes are rejected. Hermes validates placement before publishing a task and again before materializing its worktree. Idempotent retries can update an unclaimed legacy repository anchor, but cannot relabel a claimed or concurrently edited task. Existing paths outside the namespace fail closed instead of moving files. Restart the gateway after changing the policy; clearing it restores legacy placement for future resolution.

A linked project's primary repository takes precedence over the board's default repository. A missing or invalid primary repository raises an error. When another profile lacks the project record, recovery from `project_source_task_id` requires an already materialized external worktree with the recorded branch and matching Git repository identity. Conflicting board authority is rejected. An external source task that has never been materialized cannot supply this evidence; materialize it from the original profile before creating the cross-profile follow-up.
