...
- `hermes profile create NAME --auth-mode profile` 创建空的配置文件本地权限。克隆选项永不向其中复制 `auth.json` 的 OAuth 或凭据池状态；请通过独立登录或显式加密的配置文件恢复来填充该存储。其他被克隆选项选中的文件（如 `.env`）仍保持其文档所述行为。
